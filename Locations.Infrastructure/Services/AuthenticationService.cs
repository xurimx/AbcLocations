...
namespace Locations.Infrastructure.Services
{
    public class AuthenticationService : IAuthenticationService
    {
        ...
        {
            if (string.IsNullOrEmpty(refreshToken)) throw new LocationException("Refresh token or Access token is not valid. Please log in manually!");
            ...
            if (user != null)
            {
                ...
                {
                    throw new LocationException("Refresh token or Access token is not valid. Please log in manually!");
                }
            }
            throw new LocationException("Refresh token or Access token is not valid. Please log in manually!");
        }
        ...
        private ClaimsPrincipal GetPrincipalFromExpiredToken(string token)
        {
            ...
            try
            {
                ...
                if (jwtSecurityToken == null || !jwtSecurityToken.Header.Alg.Equals(SecurityAlgorithms.HmacSha256, StringComparison.InvariantCultureIgnoreCase))
                    throw new LocationException("Invalid token", 400);

                return principal;
            }
            catch (Exception e)
            {
                throw new LocationException("Refresh token or Access token is not valid. Please log in manually!", e);
            }
        }
